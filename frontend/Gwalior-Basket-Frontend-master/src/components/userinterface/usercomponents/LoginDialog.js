...
export default function LoginDialog(props) {
  ...
  const styles = {
    dialogBg: {
      background: "linear-gradient(19deg ,#21D9FD,#B721FF)",
      color: "white",
    },
    heading: {
      fontWeight: 500,
      fontSize: matches ? "1.25rem" : "2.125rem",
      maxWidth: "350px",
      lineHeight: 1.3,
      margin: matches && "80px 0px",
    },
    arrForward: {
      bgcolor: "white",
      color: "black",
    ...
    if (testOtp == otp) {
      ...
      if (result.status == 0) {
        ...
        if (result.status == 2) {
          ...
          var result_address = await postData(
            "userinterface/check_user_address",
            { mobileno: number }
          );
          if (result_address.status) {
            dispatch({
              type: "ADD_USER",
              payload: [result_address.data[0].mobileno, result_address.data],
            });
  ...
  const generateOtp = async () => {
    ...
    var result = await postData("smsapi/sendotp", {
      mobileno: number,
      otp: gotp,
    });
        ...
        {isOpen ? (
          <div>
            ...
            <div style={{ display: "flex", justifyContent: "center" }}>
              <ArrowForward
                // eslint-disable-next-line
                style={{ backgroundColor: otp.length == 6 ? "white" : "gray" }}
                onClick={handleOTP}
                sx={styles.arrForward}
              />
            </div>
          ...
          <div>
            ...
            <h2 style={styles.heading}>
              Buy Groceries and get upto 60% discount.
            </h2>
            <TextField
              fullWidth
              ...
              size="small"
              InputProps={{
                sx: { backgroundColor: "white", borderRadius: 8, py: 0.5 },
                startAdornment: (
            ...
            />
            <div style={{ color: "#8f111c", margin: "5px 0" }}>
              {errMsg.num}
            </div>
            <Button
              disabled={number.length !== 10}
              color="inherit"
              sx={{ borderRadius: 8, my: 2, py: 1.2 }}
              fullWidth
              onClick={handleClick}
              variant="outlined"
              size="large"
