...
router.post("/add_new_product", upload.single("image"), function (req, res) {
  pool.query(
    "insert into products(companyid, categoryid, productname, description, pricetype, trending, deals, image, status, createdat, updatedat, createdby) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    [
      req.body.companyid,
      req.body.categoryid,
...
router.get("/fetch_all_products", function (req, res) {
  pool.query(
    "select P.*,(select C.categoryname from categories C where C.categoryid = P.categoryid) as categoryname,(select PT.prtype from pricetyp PT where PT.pricetypid = P.pricetype) as pricetypename from products P ",
    function (error, result) {
      if (error) {
        console.log(error);
...
router.post("/edit_product", function (req, res) {
  pool.query(
    "update products set companyid = ?, categoryid = ?, productname = ?, description = ?, pricetype = ?, trending = ?, deals = ?, status = ?, updatedat = ?, createdby = ? where productid = ?",
    [
      req.body.companyid,
      req.body.categoryid,
      req.body.productname,
...
router.get("/fetch_all_pricetype", function (req, res) {
